...
export const getAllUsers = async (req, res) => {
    try {
        const users = db.users.getAll()
            ...
            .map(u => {
                const bookings = db.bookings.findByUserId(u.id);
                return {
                    id: u.id,
                    fullName: u.fullName,
                    email: u.email,
...
export const getUserProfile = async (req, res) => {
    try {
        ...
        const bookings = db.bookings.findByUserId(user.id).map(b => {
        ...
        const reports = db.interviewReports.findByUserId(user.id);
...
export const toggleBlockUser = async (req, res) => {
    try {
        ...
        const updated = db.users.update(req.params.id, {
